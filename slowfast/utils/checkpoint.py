...
def load_demo_checkpoint(cfg, model):
    """
    Loads weights of pretrained model to our newly built model.
    Works only if cfg parameters are correctly specified
    :param model: a model with randomly initialized weights, which are replaced by a pretrained model's weights
    :param cfg: the config
    """
    # Load a checkpoint if applicable.
    if cfg.ACTIONRECOGNIZER.CHECKPOINT_FILE_PATH != "":
        load_checkpoint(
            cfg.ACTIONRECOGNIZER.CHECKPOINT_FILE_PATH,
            model,
            data_parallel=False, # We can have several gpus but use only 1 gpu for the model
            optimizer=None,
            inflation=False,
            convert_from_caffe2=cfg.ACTIONRECOGNIZER.CHECKPOINT_TYPE == "caffe2",
        )
    elif has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR)
        load_checkpoint(last_checkpoint, model, cfg.NUM_GPUS > 1)
        logger.warning("No ACTIONRECOGNIZER.CHECKPOINT_FILE_PATH specified. Using last checkpoint in %s. Please update "
                       "cfg.ACTIONRECOGNIZER.CHECKPOINT_FILE_PATH." % cfg.OUTPUT_DIR)
    elif cfg.TRAIN.CHECKPOINT_FILE_PATH != "":
        # If no checkpoint found in ACTIONRECOGNIZER.CHECKPOINT_FILE_PATH or in the current
        # checkpoint folder, try to load checkpoint from
        # TRAIN.CHECKPOINT_FILE_PATH and test it.
        load_checkpoint(
            cfg.TRAIN.CHECKPOINT_FILE_PATH,
        ...
        logger.warning(
            "No ACTIONRECOGNIZER.CHECKPOINT_FILE_PATH specified. Using last checkpoint in TRAIN.CHECKPOINT_FILE_PATH "
            "%s. Please update cfg.ACTIONRECOGNIZER.CHECKPOINT_FILE_PATH." % cfg.TRAIN.CHECKPOINT_FILE_PATH)
    else:
        # raise NotImplementedError("Unknown way to load checkpoint.")
        logger.warning("Using the demo with random initialization does not make any sense. Exiting....")
        sys.exit()
...
def load_train_checkpoint(cfg, model, optimizer):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            last_checkpoint, model, cfg.NUM_GPUS > 1, optimizer, cfg=cfg
        )
